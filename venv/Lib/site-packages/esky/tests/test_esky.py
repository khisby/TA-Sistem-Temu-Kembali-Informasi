...
class TestFSTransact(unittest.TestCase):
    ...
    def assertContents(self,path,contents):
        with open(self.path(path),"rb") as f:
            self.assertEqual(f.read().decode(),contents)
    ...
    def test_move_file_with_unicode_name(self):
        self.setContents("file\N{SNOWMAN}","hello world")
        trn = FSTransaction()
        trn.move(self.path("file\N{SNOWMAN}"),self.path("file2"))
        self.assertContents("file\N{SNOWMAN}","hello world")
        self.assertFalse(os.path.exists(self.path("file2")))
        trn.commit()
        self.assertContents("file2","hello world")
        self.assertFalse(os.path.exists(self.path("file\N{SNOWMAN}")))
...
class TestPatch(unittest.TestCase):
    ...
    def test_patch_bigfile(self):
        ...
        try:
            ...
            self.assertNotEqual(dgst1,dgst2)
    ...
    def test_copying_multiple_targets_from_a_single_sibling(self):
        ...
        self.assertNotEqual(esky.patch.calculate_digest(src_dir),
        ...
        self.assertEqual(esky.patch.calculate_digest(src_dir),
                          esky.patch.calculate_digest(tgt_dir))
